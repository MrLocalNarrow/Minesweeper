...
def create_button_field(SIZE_X, SIZE_Y, root, bomb_coordinates, bomb_numbers):
    ...
    def on_right_click(x, y):
        ...
        # Spielende prüfen: Alle Bomben sind geflaggt?
        if len(flagged_coords) == bomb_numbers and all(b in flagged_coords for b in bomb_coordinates):
            end_time = time.time()
            elapsed_time = end_time - start_time
            tk.messagebox.showinfo(
                "Gewonnen!",
                f"Du hast alle Bomben gefunden!\nZeit: {elapsed_time:.2f} Sekunden"
            )
            root.destroy()
...
def aufdecken(x, y, tiles, bomb_coordinates, SIZE_X, SIZE_Y, revealed):
    if (x, y) in revealed:
        return
    revealed.add((x, y))
    ...
    if anzahl > 0:
        button.config(text=str(anzahl), state="disabled", disabledforeground="black", bg="#e0e0e0")
    else:
        button.config(text="", state="disabled", disabledforeground="black", bg="#e0e0e0")
        nachbarn = nachbarfelder_des_gecklickten_Feldes(x, y, SIZE_X - 1, SIZE_Y - 1)
        for nx, ny in nachbarn:
            if (nx, ny) not in bomb_coordinates:
                aufdecken(nx, ny, tiles, bomb_coordinates, SIZE_X, SIZE_Y, revealed)
